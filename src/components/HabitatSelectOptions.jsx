...
function HabitatSelectOptions({
  ...
  const getPokemonDataArray = async (res) => {
    res.map(async (item) => {
      ...
      dispatchFilter({ type: "FILTER_DATA_HABITAT", payload: response.data });
    });
  };
  ...
  const handleItemClick = async (item) => {
    ...
    try {
      ...
      dispatchFilter({ type: "CLEAR_DATA_HABITAT" });

      const { data } = await axios.get(
        `https://pokeapi.co/api/v2/pokemon-habitat/${item}`
      );
  ...
  return (
    <div>
      <div>
        <Button onClick={handleButtonClick}>Select a Pokemon Habitat</Button>
        {isOpen && (
          <List>
            {habitatArray.map((habitat, index) => (
              <ListItem
                key={index}
                onClick={() => handleItemClick(habitat.name)}
              >
                {habitat.name}
              </ListItem>
            ))}
...
export default HabitatSelectOptions;
